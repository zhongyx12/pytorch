...
SparseTensor new_with_dims_sparse(int64_t sparse_dim, int64_t dense_dim, ArrayRef<int64_t> size, ScalarType dtype, Layout layout, Device device, bool pin_memory) {
...
/* Shape init */
Tensor sparse_coo_tensor(ArrayRef<int64_t> size, ScalarType dtype, Layout layout, Device device, bool pin_memory) {
...
Tensor sparse_coo_tensor(const Tensor& indices, const Tensor& values_, c10::optional<ScalarType> dtype, c10::optional<Layout> layout, c10::optional<Device> device, c10::optional<bool> pin_memory) {
  ...
  // arg checking
  // TORCH_CHECK(!options.has_layout() || options.layout() == kSparse, "expected sparse layout, but got layout ", options.layout()); // [CHECK THIS]
...
Tensor _sparse_coo_tensor_unsafe(const Tensor& indices, const Tensor& values_, ArrayRef<int64_t> size, c10::optional<ScalarType> dtype, c10::optional<Layout> layout, c10::optional<Device> device, c10::optional<bool> pin_memory) {
  ...
  // arg checking
  //TORCH_CHECK(!layout.has_value() || !layout.value().has_layout() || layout.value() == kSparse, "expected sparse layout, but got layout ", layout.value()); [CHECK THIS]
